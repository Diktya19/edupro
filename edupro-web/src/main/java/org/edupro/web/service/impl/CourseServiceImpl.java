...
public class CourseServiceImpl extends BaseService implements CourseService {
    ...
    @Override
    public List<CourseSectionRes> getMaterialByCourseId(String courseId) throws EduProWebException {
        var url = backEndUrl.courseUrl()+"/"+courseId+"/material";
        return getSection(url);
    }
    ...
    @Override
    public Optional<CoursePersonResponse> savePerson(CoursePersonRequest request) {
        try {
            var url = backEndUrl.courseUrl()+"/"+request.getCourseId()+"/person";
            HttpEntity<CoursePersonRequest> httpEntity = new HttpEntity<>(request, getHeader());
            ResponseEntity<Response> response = restTemplate.postForEntity(url, httpEntity, Response.class);
            if (response.getStatusCode() == HttpStatus.OK){
                byte[] json = objectMapper.writeValueAsBytes(Objects.requireNonNull(response.getBody()).getData());
                CoursePersonResponse result = objectMapper.readValue(json, CoursePersonResponse.class);
                return Optional.of(result);
            }
